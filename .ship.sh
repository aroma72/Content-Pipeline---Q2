...
for i in $(seq 1 40); do
  ...
  case "$D" in 5333b153*SUCCESS*false) echo "deploy live"; break;; esac
done
...
for i in $(seq 1 45); do
  ...
  case "$ST" in written) echo "script READY"; break;; failed) echo "WRITE FAILED"; curl -s "$U/demo/make-video/$J" | head -c 900; exit 1;; esac
  sleep 20
done

# publish:true — Aroma has asked for a rendered, published video four times.
...
for i in $(seq 1 220); do
  ...
  L=$(echo "$R" | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{try{const j=JSON.parse(s);const p=j.produce||{};console.log(new Date().toISOString().slice(11,19)+' '+j.status+' | '+(p.stage||'')+' '+(p.error?('ERR '+p.error.slice(0,300)):'')+' '+(p.youtube?JSON.stringify(p.youtube):''))}catch(e){console.log('poll error')}})")
  echo "$L"
  case "$L" in *youtu*) echo "=== PUBLISHED ==="; exit 0;; *"| failed"*|*awaiting_review*) exit 2;; esac
  sleep 30
done
