...
| 2c | `LESSON_HTML=… node qa-frames.js` | **After TTS, before compile.** Opens the REAL renderer with the real beats and measures computed type: nothing under 28px, every text-only slide leading at 60px+, no blank frames, nothing off-canvas. Catches the missing-stylesheet class that every other gate is blind to (2026-09-17). |
| 2b | `node qa-info.js && node qa-checkpoint.js && node qa-visuals.js` · `node qa-cutouts.js` | **REQUIRED before you spend a rupee.** §3d visual standard (one concrete setting · AI is a laptop, never a glowing orb · ≥28% `scene` · ≤2 `statement` cards · ≥3 data templates + real numbers · scenes show an action) and no half-cut props. Exit 2 → fix `beats.js` first |
