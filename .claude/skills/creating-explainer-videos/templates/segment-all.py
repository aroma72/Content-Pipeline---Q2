...
def foreground_mask(rgb: np.ndarray) -> np.ndarray:
    """True where pixel is the subject (not near-white/cream background)."""
    r, g, b = rgb[..., 0].astype(int), rgb[..., 1].astype(int), rgb[..., 2].astype(int)
    mx = np.maximum(np.maximum(r, g), b)
    mn = np.minimum(np.minimum(r, g), b)
    brightness = mx
    saturation = (mx - mn)
    # background = bright AND low saturation (white/cream). Everything else = subject.
    bg = (brightness > 205) & (saturation < 28)
    return ~bg
