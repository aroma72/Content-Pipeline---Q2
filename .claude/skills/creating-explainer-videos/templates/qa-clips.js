...
for (const f of clips.sort()) {
  ...
  // ---- 3b. INVENTION / SUBJECT LOSS (clip vs the source art) ----
  // The comparison above looks WITHIN the clip, so it cannot see this: a clip that calmly
  // paints a floating object onto a plain wall, or removes the protagonist altogether, is
  // perfectly consistent frame to frame and passes every check. Both shipped on 2026-09-15
  // (notion-slack-01 beat 13: a blank sheet invented against an empty wall; the retry deleted
  // Ali and the laptop entirely). i2v starts FROM the art, so the final frame should still
  // look like it — a low score means the model rebuilt the shot rather than moved it.
  let inv = NaN;
  if (fs.existsSync(art)) {
    const last = path.join(tmpdir, id + '_last.png');
    if (grab(clip, Math.max(0.5, cd - 0.3), last)) {
      inv = ssim(art, last);
      if (!isNaN(inv) && inv < 0.45) {
        issues.push('INVENTED (ssim ' + inv.toFixed(3) + ' vs art) — content added or removed; '
          + 'animate a denser shot instead, a large plain wall invites invention');
      }
    }
  }
  ...
  const fatal = issues.some((i) => /FROZEN|MORPHED|INVENTED/.test(i));
