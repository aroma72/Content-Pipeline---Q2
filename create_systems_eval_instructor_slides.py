"""
Generate Systems Evaluations Instructor Slides - Clean, Minimal Design
Professional presentation with integrated images - FIXED VERSION
"""
...
def create_placeholder_image(width=600, height=400, color_rgb=(31, 78, 121)):
...
def get_image(url, width=600, height=400, fallback_color=(31, 78, 121)):
    """Get image from URL with fallback to placeholder"""
    try:
        ...
        if response.status_code == 200:
            ...
            img.save(img_bytes, format='PNG')
            img_bytes.seek(0)
            print(f"✓ Loaded image from {url[:40]}...")
            return img_bytes
    except Exception as e:
        print(f"✗ Failed to load {url[:40]}... Using placeholder")

    return create_placeholder_image(width, height, fallback_color)
...
def add_accent_line(slide, top_inches=0.8):
    """Add accent line under header"""
    line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, Inches(top_inches), Inches(10), Inches(0.05))
    line.fill.solid()
...
def create_presentation():
    ...
    # Right content box with background
    box_shape = slide2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(5.2), Inches(1.2), Inches(4.3), Inches(5.7))
    box_shape.fill.solid()
    box_shape.fill.fore_color.rgb = LIGHT_BG
    box_shape.line.color.rgb = ACCENT_COLOR
    ...
    # Add image
    img_data = get_image("https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=600&q=80", fallback_color=(31, 78, 121))
    add_image_safe(slide3, img_data, Inches(5.5), Inches(1.2), width=Inches(4), height=Inches(5.8))
...
• Best of both worlds"""
    ...
    add_text_box(slide3, Inches(0.4), Inches(1.2), Inches(5), Inches(5.8),
                content, font_size=14, color=TEXT_DARK)
    ...
    # Add image
    img_data = get_image("https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=600&q=80", fallback_color=(0, 176, 176))
    add_image_safe(slide4, img_data, Inches(0.4), Inches(1.2), width=Inches(4.5), height=Inches(5.8))
    ...
    for title, desc in boxes_content:
    ...
    img_data = get_image("https://images.unsplash.com/photo-1516321318423-f06f70d504f0?w=600&q=80", fallback_color=(192, 0, 0))
    add_image_safe(slide5, img_data, Inches(5.5), Inches(1.2), width=Inches(4), height=Inches(5.8))

    rubric_content = """Rubric Components:
...
Real sample outputs"""
    ...
    add_text_box(slide5, Inches(0.4), Inches(1.2), Inches(5), Inches(5.8),
                rubric_content, font_size=15, color=TEXT_DARK)
    ...
    img_data = get_image("https://images.unsplash.com/photo-1552664730-d307ca884978?w=600&q=80", fallback_color=(0, 176, 176))
    add_image_safe(slide6, img_data, Inches(5.5), Inches(1.2), width=Inches(4), height=Inches(5.8))

    tips_content = """1. Be Specific
...
5. Iterate
Use feedback to refine rubrics"""

    add_text_box(slide6, Inches(0.4), Inches(1.2), Inches(5), Inches(5.8),
                tips_content, font_size=14, color=TEXT_DARK)
    ...
    img_data = get_image("https://images.unsplash.com/photo-1552664730-d307ca884978?w=600&q=80", fallback_color=(31, 78, 121))
    add_image_safe(slide8, img_data, Inches(5.5), Inches(1.2), width=Inches(4), height=Inches(5.8))

    next_steps = """Step 1: Define Goals
...
Step 5: Scale
Apply framework to full dataset"""

    add_text_box(slide8, Inches(0.4), Inches(1.2), Inches(5), Inches(5.8),
                next_steps, font_size=14, color=TEXT_DARK)
...
print("\n🎯 Creating Systems Evaluations Instructor Slides...")
print("=" * 60)
...
print("=" * 60)
print(f"\n✅ PowerPoint created successfully: {output_path}")
print(f"   • 9 professional slides")
print(f"   • Clean, minimal design with rounded corners")
print(f"   • Images integrated on relevant slides")
print(f"   • Professional color scheme (navy + teal)")
print(f"   • Professional formatting with proper spacing")
print(f"   • Ready for instructor delivery\n")
