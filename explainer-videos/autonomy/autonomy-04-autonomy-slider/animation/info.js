...
(function () {
  ...
  // ---- autonomy-04: the 1-10 Autonomy Slider (dial). Pointer knob at data.value;
  // optional labels under specific numbers. Deterministic (no time dependence).
  //   data:{ value, labels?:{1:'..',6:'..',10:'..'} }
  T.dial = (d) => {
    const wrap = el('div', 'dial');
    const scale = el('div', 'dial-scale seq');
    scale.appendChild(el('div', 'dial-bar'));
    const ptr = el('div', 'dial-ptr');
    ptr.style.left = (((d.value || 1) - 0.5) / 10 * 100) + '%';
    scale.appendChild(ptr);
    wrap.appendChild(scale);
    const nums = el('div', 'dial-nums');
    for (let i = 1; i <= 10; i++) nums.appendChild(el('div', 'dial-num' + (d.value === i ? ' on' : ''), String(i)));
    wrap.appendChild(nums);
    if (d.labels) {
      const row = el('div', 'dial-labels');
      for (let i = 1; i <= 10; i++) {
        const cell = el('div', 'dial-lab' + (d.labels[i] ? ' has' : ''));
        if (d.labels[i]) cell.textContent = d.labels[i];
