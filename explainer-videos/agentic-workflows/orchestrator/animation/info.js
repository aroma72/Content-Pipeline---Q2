...
(function () {
  ...
  // ---- autonomy series: interactive multiple-choice QUESTION / REVEAL card ----
  // Same template serves both: QUESTION (no answer) and REVEAL (answer highlights
  // that option). Options reveal via .seq so it is deterministic under seekTo().
  //   data:{ stem, options:[...], answer?:index-to-highlight, note? }
  T.quiz = (d) => {
    const wrap = el('div', 'quiz');
    if (d.stem) wrap.appendChild(el('div', 'quiz-stem seq', d.stem));
    const opts = el('div', 'quiz-opts');
    (d.options || []).forEach((o, i) => {
      const row = el('div', 'quiz-opt seq' + (d.answer != null && i === d.answer ? ' correct' : ''));
      row.appendChild(el('span', 'quiz-badge', String.fromCharCode(65 + i)));
      row.appendChild(el('span', 'quiz-text', o));
      opts.appendChild(row);
    });
    wrap.appendChild(opts);
    if (d.note) wrap.appendChild(el('div', 'quiz-note seq', d.note));
    return wrap;
  };

  // ---- agentic-workflows: orchestrator diagram. One lead node + worker nodes;
  // arrows fan OUT (split, dir:'out') or come IN (gather, dir:'in'). Deterministic.
  //   data:{ lead, workers:[...], dir:'out'|'in' }
  T.orchestrator = (d) => {
    const gather = d.dir === 'in';
    const wrap = el('div', 'orch' + (gather ? ' gather' : ' split'));
    wrap.appendChild(el('div', 'orch-lead seq', d.lead || 'Lead'));
    const arrows = el('div', 'orch-arrows seq');
    (d.workers || []).forEach(() => arrows.appendChild(el('div', 'orch-arrow', gather ? '↑' : '↓')));
    wrap.appendChild(arrows);
    const row = el('div', 'orch-workers');
    (d.workers || []).forEach((w) => row.appendChild(el('div', 'orch-worker seq', w)));
    wrap.appendChild(row);
    return wrap;
  };

  // ---- agentic-workflows intro: a menu of workflow patterns, one highlighted ----
  //   data:{ items:[{name, brief, hi?}] }
  T.menu = (d) => {
    const wrap = el('div', 'menu');
    (d.items || []).forEach((it) => {
      const row = el('div', 'menu-row seq' + (it.hi ? ' hi' : ''));
      row.appendChild(el('div', 'menu-name', it.name || ''));
      row.appendChild(el('div', 'menu-brief', it.brief || ''));
      wrap.appendChild(row);
    });
    return wrap;
  };
