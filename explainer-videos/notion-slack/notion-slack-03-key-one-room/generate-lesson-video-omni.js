...
// Motion direction comes from the BEAT itself (`motion` in beats.js), not from a map
// in this file. A hardcoded map is copied along with the folder and beat ids repeat
// across videos, so a scaffolded video silently animated the PREVIOUS video's story --
// a trap that only shows up when you watch the finished render. Keeping the motion
// beside the sentence it belongs to makes that impossible.
//
// MOTION below stays only as a fallback for hand-made videos written before `motion`
// existed. New videos should leave it empty.
const MOTION = {};

// Motions stay SMALL and in-character: i2v drifts and morphs the flat 2D art on big movement.
...
// Default to the beats that actually asked for motion. Animating EVERY art beat costs
// more than the rest of the video combined and reads as busy; the house rule is real
// movement on the 2-4 beats where it carries the story. ART_IDS still overrides, and
// ANIMATE_ALL=1 restores the old blanket behaviour for a deliberate experiment.
const wantMotion = artBeats.filter((b) => b.motion);
const todo = onlyIds.length
  ? artBeats.filter((b) => onlyIds.includes(b.id))
  : (process.env.ANIMATE_ALL === '1' ? artBeats : (wantMotion.length ? wantMotion : []));
...
(async () => {
  if (!todo.length) {
    console.log('[i2v] no beat asks for motion (set `motion` on 2-4 beats in beats.js, ' +
                'or ART_IDS=.. / ANIMATE_ALL=1) -- leaving the stills to the Ken Burns camera.');
    return;
  }
  ...
  for (const b of todo) {
    ...
    try {
      ...
      const prompt = ((b.motion || MOTION[b.id] || DEFAULT_MOTION) + STYLE_LOCK);
