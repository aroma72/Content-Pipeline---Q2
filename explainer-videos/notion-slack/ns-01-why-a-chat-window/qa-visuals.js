...
// Technical how-to format (established 2026-09-16, Aroma: "leave the Ali storyline altogether").
// No protagonist, no illustrated setting — the whole video is interface screens, diagrams and
// terminal output rendered as HTML. Every rule below is about character art, so none of them apply.
// Detected rather than declared: a video with no `ali` and no `scene` beat has no character to check.
const hasCharacterArt = beats.some((b) => b.mode === 'ali' || b.mode === 'scene');
if (!hasCharacterArt) {
  console.log('[qa-visuals] ⏭  technical how-to format — no character art, so the §3d character rules '
    + 'do not apply. The bar here is qa-info (card data shapes) and qa-checkpoint (the question).');
  process.exit(0);
}
...
// ---- 7. PROMPT LINT — phrasings that reliably render wrong ----------------
// Added 2026-09-15 from defects Aroma found watching notion-slack-01.
// Strip the HERO guard clause first — it deliberately contains 'never one eye closed',
// and matching your own guardrail is how a gate gets switched off in annoyance.
const GUARD = /BOTH EYES OPEN[^,]*,?\s*never winking,?\s*never one eye closed/ig;
const artOf = (b) => (typeof b.art === 'string' ? b.art.replace(GUARD, '') : '');

// A face described asymmetrically comes back winking. "one eyebrow slightly raised" produced a
// closed eye on beat 06 — unsettling on a 5-second hold of a friendly character.
const WINKY = /\bone (eyebrow|eye)\b|\bwink|\bhalf[- ]closed\b|\braised eyebrow\b/i;
const winky = beats.filter((b) => WINKY.test(artOf(b)));
if (winky.length) {
  fail('WINK', `beats ${winky.map((b) => b.id).join(', ')} describe the face asymmetrically ` +
    '("one eyebrow raised", "one eye…") — the model renders this as a WINK. Say the feeling ' +
    '("gently amused", "rueful") and keep the HERO const\'s both-eyes-open clause.');
}

// Background people rendered as small child-like figures clustered at a counter — Aroma read them
// as "4 kids". Any figure in a prompt must be explicitly adult-proportioned.
const PEOPLE = /\b(staff|assistants?|colleagues?|customers?|people|figures?|workers?)\b/i;
const ADULT = /\badult\b/i;
const childish = beats.filter((b) => PEOPLE.test(artOf(b)) && !ADULT.test(artOf(b)));
if (childish.length) {
  fail('FIGURES', `beats ${childish.map((b) => b.id).join(', ')} put people in the frame without ` +
    'saying they are adults — they come back child-sized. Write "adult … of full adult height ' +
    'and adult proportions", or cut the figures if they carry no meaning in that beat.');
}

// Every animated beat must own real art to animate FROM; i2v invents content in empty space.
const ids = new Set(beats.map((b) => b.id));
const animate = Array.isArray(module.parent ? null : null) ? [] : (require(path.join(process.cwd(), 'beats.js')).animateIds || []);
const dangling = animate.filter((id) => !ids.has(id));
if (dangling.length) {
  fail('ANIMATE', `animateIds names ${dangling.join(', ')}, which are not beats in this video`);
}
const noArt = animate.filter((id) => { const b = beats.find((x) => x.id === id); return b && !artOf(b); });
if (noArt.length) {
  fail('ANIMATE', `animateIds names ${noArt.join(', ')}, which have no art — i2v needs a source image`);
}
